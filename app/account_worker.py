...
def _apply_publish_decision(
    ...
    with closing(_connect(db_path)) as conn:
        if decision["status"] == "published":
            ...
            # 交给定时同步,不重试。
            from app.services.note_ledger import (
                record_published_note,
                schedule_note_ledger_sync,
            )
            record_published_note(db_path, job_id)
...
def _resolve_execute(kind: str) -> Callable[[Optional[int], dict], Any]:
    ...
    if kind in ("note_comment", "note_comment_task"):
        # 发布后评论(note_comment_task)与手工单篇评论(note_comment)执行契约完全一致
        # (payload 同形、浏览器动作同一套),只是登记入口不同,故复用同一个 execute。
    ...
    if kind == "audience_sync":
        from app.services import audience_sync

        return lambda account_id, payload: audience_sync.execute(account_id, payload)
    ...
    if kind == "note_extract_comments":
        from app.services import note_extract_comments

        return lambda account_id, payload: note_extract_comments.execute(
            account_id, payload
        )
    ...
    if kind == "note_collection_create":
        from app.services import note_collection_create

        return lambda account_id, payload: note_collection_create.execute(account_id, payload)
