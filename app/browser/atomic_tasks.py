...
# 正文+标签的**平台硬天花板**。与上面 900 的关系:900 是我们**自己发新笔记**时留出标签
# 余量的安全线(标签另占约 60,发布链路照旧按 900 截断,不受本常量影响);1000 才是平台
# 真正的收口线。两者必须分开,是因为**编辑既存笔记**时会撞见平台允许、我们的安全线拒收的
# 笔记 —— 2026-08-09 内容运营取证:号8 笔记 6a6b3c4c 线上正文 937 字、号1 6a6a0136 925 字,
# 平台上活得好好的,而 REST 的 max_length=900 让这几篇**任何**写回都进不来(连"把 ¥800
# 改成 ¥600"这种缩短正文的改动都被拒),调价动作被整个卡死。编辑期改用本常量放行,
# "不许比原来更长"由 ``apply_content_edit`` 的 max(900, L0) 判据兜住。
XHS_ABS_BODY_LIMIT = 1000
...
class XHSPublishAtomicTasks:
    ...
    def step2a_upload_audio(
        ...
        点按钮前先尽力关引导浮层:实测「播客合集上线啦」的 popover 正压在这颗按钮上。
        **关不掉是常态**(08-08 取证四招全无效),所以真正走通的不是关、是**绕** ——
        浮层没关掉时改点按钮右侧那条没被压住的暴露缝穿透它(``upload_audio_click_point``);
        缝算不出来(尺寸/形态变了)才退回直接点按钮元素。
    ...
    def _set_audio_cover(self, cover_path: Optional[str], audio_input) -> Dict[str, Any]:
        ...
        if cover_input is None:
            logger.warning(
                "[step2a] 弹窗里认不出音频封面的 file input —— 两个 input 靠 accept 区分"
                "(封面 accept 含图片扩展名,已取证),这一刻却没有一个合格候选,本次不设封面 "
                "—— 笔记照发"
            )
    ...
    def step3a_wait_for_audio_upload(
        ...
        就是"平台认为音频到位了"的最直接信号。禁用态形态**已取证**(08-08,账号9:解禁时
        ``disabled`` 属性消失、class 不含任何 disabled 字样),判据收在纯函数
        ``go_publish_enabled`` 里;那里仍把 ``disabled`` 属性 / ``aria-disabled`` / class 含
        disabled 三路都读、**全都不表示禁用**才算可点(取"与"而不是"或":多读两路成本为零,
        平台哪天改成挂 class 也不漏判;读不懂的形态一律当禁用,宁可等到超时也不点一颗可能
        禁用的按钮 —— 图文那边 2026-08-02 就栽在点禁用按钮换来一句"发布超时")。
