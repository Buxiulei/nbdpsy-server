...
# 快照采集的属性名(单一事实源:下方 JS 内联列表 = 本常量,测试对此断言)。
# contenteditable 必须在列:passes_safety 输入类靠 "contenteditable" in attrs 放行,
# 富文本编辑器(如 div[contenteditable='true'])的自愈依赖它。
_SNAPSHOT_ATTR_NAMES = [
    "id", "class", "name", "type", "placeholder",
    "aria-label", "data-testid", "contenteditable",
]
...
(sel) => {
  ...
  for (const node of nodes) {
    ...
    ref += 1;
    const attrNames = __ATTR_NAMES__;
    const attrs = {};
...
""".replace("__ATTR_NAMES__", json.dumps(_SNAPSHOT_ATTR_NAMES))
...
def _bbox_center_matches(box_a: dict | None, box_b: dict | None, tol: float = 8.0) -> bool:
    """比对两个 bbox 中心点是否同址(x/y 各允许 tol px 容差)。任一为 None → False。

    box 形如 {x, y, width, height}(视口坐标,与 getBoundingClientRect / Playwright
    bounding_box 同系)。用于校验 query_selector 命中的 handle 是否就是 LLM 选中的
    快照元素——退化选择器(tag.class / 裸 tag)常命中同类别的另一个元素,中心点错位
    即判定不是同一元素,弃该选择器走坐标兜底。异常/缺字段 → False(保守判不匹配)。
    """
    try:
        if not box_a or not box_b:
            return False
        ax = box_a.get("x", 0) + box_a.get("width", 0) / 2
        ay = box_a.get("y", 0) + box_a.get("height", 0) / 2
        bx = box_b.get("x", 0) + box_b.get("width", 0) / 2
        by = box_b.get("y", 0) + box_b.get("height", 0) / 2
...
class SelfHealLocator:
    ...
    def locate(self, page, intent_key: str, intent_desc: str) -> tuple | None:
        ...
        try:
            ...
            # 优先用派生选择器取 handle;命中且 bbox 中心与快照吻合才当"可学"选择器返回。
            if selector:
                try:
                ...
                if handle is not None:
                    # 退化选择器(tag.class / 裸 tag)可能命中同类别的另一个元素——
                    # 比对命中 handle 与快照 el 的 bbox 中心,错位则弃选择器走坐标兜底,
                    # 避免把"能解析但指错"的坏选择器传给下游 learn 进 registry。
                    try:
                        handle_box = handle.bounding_box()
                    except Exception:
                        handle_box = None
                    if _bbox_center_matches(handle_box, el.get("bbox")):
                        selector_out = selector
                    else:
                        logger.warning(
                            f"[self_heal] 选择器 {selector} 命中元素与快照 bbox 错位,"
                            "弃用走坐标兜底"
                        )
                        handle = None
