...
# ⚠️ 三个流量指标**口径互不相同,不可互相推算**(2026-07-26 悬停数据看板 ⓘ 读到的官方原文):
#   - 曝光:「每天更新数据,该篇笔记**截至昨日**获得的曝光数之和」
#   - 观看:「**实时更新数据**,笔记**截止目前**的观看数」
#   - 封面点击率:「每天更新数据,该篇笔记**截至昨日**的**封面点击次数÷封面曝光次数**
#     (视频下滑不计入)」
# 故 `观看 / 曝光` **算不出** cover_ctr,三点原因叠加:①时间窗错位(观看是实时 T、
# 曝光与 ctr 是 T-1)②分子不同(ctr 分子是封面点击次数,点了秒退计点击不计观看)
# ③ctr 排除视频下滑曝光。实测两号偏差方向相反正由此而来:还在涨观看的新笔记
# 「观看/曝光」被抬高(显得 ctr 偏低),不再涨的老笔记则「点击>观看」显得 ctr 偏高。
# **cover_ctr 字段本身自洽可信,不要用 观看/曝光 去"校验"它。**
