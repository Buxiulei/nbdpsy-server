...
MANIFEST_ENTRIES = [
    {
        ...
        "notes": "**拿不到的字段一律给 null 并在 unavailable 里说明原因,绝不静默省略键**。要点:"
                 ...
                 "完全不碰浏览器 —— 但**请自觉控制频率**(连续拆解多篇时建议间隔数秒):这条路"
                 "不烧账号会话,暴露的是**我方服务器 IP**,一口气打几百次是把服务器往风控里推;"
                 "同一 note_id 24h 内直接走缓存不重复抓(重复请求同一篇不算频率);"
    ...
    },
    {
        ...
        "notes": "status 五态:queued / running / done / error(reason 给原因,如 wall_scan_qr="
                 "该号撞了验证墙需手机扫码)/ unknown。**幂等(纯只读抓取),但 reason 以 wall_ "
                 "开头时禁止重发** —— 撞墙立刻重发正是把这个号往风控深处推的打法,先按墙型"
                 "处置该号(wall_scan_qr=人工手机扫码解墙,解完再发);其余 error 可直接重发。"
                 "complete=false 表示没抓到底(轮数封顶),comments 里是已抓到的那些;"
                 "stop_reason 说明为什么停(reached_limit=拿够了 / no_new_after_scroll=到底了 / "
                 "reached_expected_total=已抓够平台标称的评论总数 / round_cap=轮数封顶 / "
                 "empty=确实没有评论 / empty_but_expected=**一条都没读到、但平台标称有评论**"
                 "——选择器多半整体失配了,此时 complete=false 且带 expected_total,"
                 "**别当成「这篇没人评论」**,请报给开发重新取证)。"
                 "⚠️ 两条读数纪律:① 服务端已用 interact.comment(平台标称评论数)替你做了这层"
                 "交叉验证(即上面的 empty_but_expected);拿到 stop_reason=empty 才是真没人评论;"
                 "② **comments 只含一楼 + 默认展开的子回复,条数天然少于 "
