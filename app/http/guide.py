...
CAPABILITY_GROUPS = [
    ...
    },
    {
        "key": "audience",
        "title": "受众行为库",
        "summary": "**谁在跟我们矩阵互动**:主站通知页采回来的赞/收藏/关注事件流,"
                   "按互动者聚合成身份、纵向轨迹、群体切片与潜客漏斗。"
                   "全部端点**纯 DB 读**,不起浏览器、不烧任何账号会话额度,可随意调。"
                   "⚠️ 两条读它必须知道的事:①**潜客分是 v1 启发式不是概率** —— 转化回流"
                   "数据(谁最终进了私域)当前不存在,权重是运营直觉的初版,分数只是候选人群内"
                   "可解释的相对排序;②**这是受众公开行为分析,不是个人档案** —— 只有平台通知流"
                   "已公开给我们的字段(userid/昵称/头像/关系/公开笔记),库里没有也不会有"
                   "userid → 来访者真实身份(姓名/手机/预约/咨询关系)的关联,也不采集互动者主页。"
                   "自家矩阵号默认剔除(不剔的话「最活跃受众」永远是自家号互刷出来的量)。",
        "paths": [
...
CHANGELOG_ENTRIES = [
    {
        "date": "2026-08-14",
        "title": "引用挂载真因根治:点击落在弹窗外;两类失败码不再意味着「换目标」",
        "kind": "fix",
        "summary": "此前「确认钮不解禁」「平台弹非公开提示」两类引用失败,实测真因是目标卡"
                   "在弹窗滚动容器的可视区外,点击根本没落在卡上(人工在同一目标上引用成功,"
                   "反证平台并未禁止)。现在滚不进可视区会明确报 quote_card_offscreen 并带"
                   "卡片/容器两组矩形证据。⚠️ 此前文档说 quote_target_not_quotable「重试"
                   "无用请换目标」是误判,已撤销——遇到这两类失败先升级到本版本重试,"
                   "别急着换引用目标。另:补量选篇顺序已按业务指令改为「零互动笔记优先,"
                   "各级内按发布时间新到老」,老积压沉队尾最后清属设计意图。",
        "endpoints": [
            "/api/accounts/{account_id}/note-components",
            "/api/note-components/{job_id}",
        ],
    ...
    {
        "date": "2026-08-14",
        "title": "引用挂载第二版修复:候选翻页到底 + 候选卡不在可视区就不点",
        "kind": "fix",
        "summary": "上一版翻页修复没真正生效——滚动落点选错,第一轮之后就被当成"
                   "「列表到底了」,笔记多的号仍然只翻到二十来篇。现在落点改成弹窗里真正"
                   "可滚的那个容器,能一路翻到候选上限。另外三件事:①目标候选卡没滚进列表"
                   "可视区时**不再照着区外坐标点**(点了也命中不到卡),报 quote_card_offscreen "
                   "并带卡片矩形与可视区矩形两组数——这才是此前「确认钮不解禁」的真因;"
                   "②点卡后若出现「非公开可见笔记，无法引用」提示,仍报 "
                   "quote_target_not_quotable 并带原文,但**不再断言是平台裁决/重试无用/该换"
                   "目标**:2026-08-14 人工在同一目标上手动引用成功已证伪该说法,这条提示可能"
                   "来自点击落在候选卡之外命中的其它元素,提示容器还会累加历史通知;"
                   "③失败回执新增候选覆盖面"
                   "(翻到多少篇 / 最老一篇 / 滚了几轮),用来分辨「目标不存在」与「目标在平台"
                   "候选窗口外」——平台给候选列表设了上限(实测约 50 篇、按时间倒序),更老的"
                   "笔记永远翻不到,那是换目标而不是重试能解决的。",
        "endpoints": [
            "/api/accounts/{account_id}/note-components",
            "/api/note-components/{job_id}",
        ],
    ...
    {
        "date": "2026-08-13",
        "title": "引用笔记挂载修复:深位候选翻页到齐 + 不再点禁用的确认按钮",
        "kind": "fix",
        "summary": "此前引用弹窗候选只加载第一页(懒加载没人滚),笔记多的号引用自己排位"
                   "靠后的笔记必然报「不在候选」,还会被错误降级到他人笔记检索(必空);"
                   "另有选卡未生效仍点确认(按钮其实是禁用态)导致的假挂载。两处已修:"
                   "候选自动翻页直到找到目标或真到底;选卡后校验确认钮解禁、点确认前查"
                   "禁用态。调用方无需改动;若再见「不在候选」类失败,新错误码会明确区分"
                   "「翻完了确实没有」与「选中没生效」,别再按旧经验猜。",
        "endpoints": [
            "/api/accounts/{account_id}/note-components",
            "/api/note-components/{job_id}",
        ],
    ...
    {
        "date": "2026-08-13",
        "title": "按标题删除加同名硬闸:同题多篇默认拒绝,防删错不可逆",
        "kind": "fix",
        "summary": "删除按管理页顺序取第一张同题卡=最新那篇;同题两篇一死一活的场景"
                   "(重复发布残留)会精确删掉要保留的健康篇。现在同题卡 ≥2 张时默认拒绝"
                   "并报 ambiguous_title;确系「删 N 留 1」的同题清理,请求体显式带"
                   " allow_ambiguous=true 放行。单题唯一的删除完全不受影响。",
        "endpoints": [
            "/api/accounts/{account_id}/note-deletions",
        ],
    },
    ...
    },
    {
        ...
        "kind": "fix",
        "summary": "补量原有四层闸管的是「补得多快」,管不了「白开多少次注定失败的会话」。"
                   "实据:一个号自 08-08 起 96 连败,全是同一个 profile_not_loaded"
                   "(登录态在但打不开任何发布者主页),因为没撞验证墙,「撞墙即停」那套一次都没"
                   "触发,冷却一过就再试;同期三篇 views=0 的笔记被全部 9 个号报「主页找不到」"
                   "(大概率平台屏蔽),各积十几个 error 还在重试。现在两个断路器都在选篇阶段判:"
                   "①连续 N 次(默认 6 条台账行 ≈ 三篇整篇失败)全失败的号本轮不派活,"
                   "12h 后半开探测放行一轮(只做 1 篇),成功即自动复位;"
                   "②被 ≥3 个**有资格**(valid 且未被 ① 熔断)的不同号在发布者主页里翻不到的篇"
                   "**永久**移出候选。对调用方的变化只有一处**纯新增**:POST 回执多一个 "
                   "suppressed_notes 字段,列出被熔断的 note_id——⚠️ **这是要人去平台核实的清单**,"
                   "不是错误,系统不会自动恢复它们。翻不到有两种成因、系统区分不了(被平台屏蔽,"
                   "或它在主页里排太靠后翻不到),所以请逐篇看过再判;确认某篇又该补了,"
                   "删掉 note_interactions 里该 note_id 的 error 行即可重新入池。触发回执可能比"
                   '以前更常出现 status="skipped"(号被熔断时不建空转任务),那是预期行为不是故障。',
        "endpoints": [
            "/api/interaction-backfills",
            "/api/interaction-backfills/{job_id}",
        ],
    ...
    },
    {
        ...
        "kind": "feature",
        "summary": "此前系统只知道「我们发了什么」,不知道「谁在看、谁反复来、谁快要转化」——"
                   "平台通知页里那些赞/收藏/关注事件看过即散,没有任何地方留下来。现在有了:"
                   "后台采集单(kind=audience_sync)把主站通知页的赞/收藏/关注事件流落库,"
                   "五个**纯 DB 读**端点在上面做分析,不起浏览器、不烧任何账号会话额度。"
                   "①`/api/audience/overview` 汇总卡(互动者总数、关系分布、近 7/30 天新增、高潜人数);"
                   "②`/api/audience/actors` 互动者列表,可按潜客分/互动次数/最近互动排序,"
                   "按关系(fstatus)、是否关注了我(followed)、互动类型筛;"
                   "③`/api/audience/actors/{userid}` 单人**纵向轨迹**:事件时间线 + 跨自家号分布 + "
                   "关系演变**变化点**(none→fans 那一刻就是这个人被内容打动的时刻)+ 潜客分明细;"
                   "④`/api/audience/funnel` 五层潜客漏斗(陌生轻触/**陌生高频**/已关注浅互动/"
                   "已关注高频/互关)+ 各层代表人物,重点是「陌生高频」那层——反复来看却始终没关注,"
                   "正是「高互动意愿但还没转化」的人;⑤`/api/audience/segments` 群体切片"
                   "(关系分布/活跃度分档/互动类型/最受互动的笔记)。"
                   "⚠️ **潜客分是 v1 启发式不是概率**:转化回流数据(谁最终进了私域)当前不存在,"
                   "五维权重(次数/跨号/最近度/深度/关系)是运营直觉的初版,分数在**候选人群内** "
                   "min-max 归一化后加权,所以它是相对位置、跨账号范围与跨时间都不可比,"
                   "别拿它当「这个人有多大概率来咨询」。分项明细(raw/normalized/weight)随单人页返回,"
                   "可解释不是黑盒。"
                   "⚠️ **这是受众公开行为分析,不是个人档案**:只含平台通知流已公开给我们的字段,"
                   "库里没有也不会有 userid → 来访者真实身份(姓名/手机/预约/咨询关系)的关联,"
                   "不采集互动者主页。自家矩阵号默认剔除(exclude_self=false 可看自家互刷量)。"
                   "数据只有**赞/收藏/关注**,评论和@ 本版不采。",
        "endpoints": [
            "/api/audience/overview",
            "/api/audience/actors",
            "/api/audience/actors/{userid}",
    ...
    },
    {
        ...
        "summary": "内容运营调价 4 条既存长笔记时被整个卡死,三个独立缺陷叠加逐条修,"
                   "对调用方**只放宽不收紧**:"
                   "①**编辑既存超 900 字笔记的正文放行**:平台上真有 925/937 字的存量笔记,"
                   "`content` 一刀切 ≤900 让它们连「把 ¥800 改成 ¥600」这种缩短正文的改动都提不"
                   "进来。REST 层 `content` 上限放宽到硬天花板 1000;`max(900, 原文长度)`"
                   "(**只许不变长**)这一档在浏览器层判(REST 拿不到原文长度),不合格时该步"
                   "`content_too_long_for_note`、**零点击零清空**、笔记原样未动。"
                   "②**长正文清空修复**:900 字多节点长文旧实现固定 3 轮全选删除 + 40 键退格预算"
                   "不够,会误报「清空失败」弃提交;改成看进度不看次数、退格预算随残留长度走。"
                   "③**话题分隔归一化(防丢话题)**:台账 `content_text` 里话题是**空格分隔**"
                   "`#A[话题]# #B[话题]#`,而正文注入要求**连写** `#A[话题]##B[话题]#` 才认得出"
                   "chip,直接拿 content_text 回写会丢话题(实测一篇丢 8 个);注入前加纯字符串"
                   "归一,空格分隔与连写两种输入都不丢。"
                   "④`published-notes` 的 `content_text` 字段说明补上「**压平文本(换行→空格)、"
                   "非可写回原文**,要可写回原文走 `POST /api/notes/extract` 的 content」。",
        "endpoints": [
            "/api/accounts/{account_id}/note-components",
            "/api/published-notes/{note_id}",
        ],
    ...
    {
        "date": "2026-08-11",
        "title": "广播默认口径终裁更正(纯文档):默认单号定向,广播仅限全矩阵品牌位",
        "kind": "fix",
        "summary": "0.22.0 上线时的运营口径「除非特指默认全代管齐发」被老板终裁推翻——内容线"
                   "以「同稿六号与账号定位方案串台+广播一次吃 6 倍每日上限互相蚕食」上交裁决,"
                   "终裁为**默认单号定向发布,广播仅用于全矩阵品牌位内容**。广播能力本身与 API"
                   " 行为**零变化**(不传 account_id 仍然=广播),变的只是推荐默认动作;"
                   "manifest 的 publish-jobs notes 已同步更正,按旧口径行事的调用方请更新。",
    ...
    },
    {
        ...
        "summary": "三件事。①**POST /api/publish-jobs 的 account_id 转可选**:省略即"
                   "**广播给全部代管账号**(每号各一条独立任务,响应形状变成 "
                   "{broadcast:true, jobs:[{account_id, job_id}...]}),传了则与以前一字不变 "
                   "—— 老调用**完全不受影响**,这是纯增。②账号有了 managed(代管/水军)与 "
                   "note_cap(笔记上限,默认 100)两个属性,读写见 GET /api/managed-accounts 与 "
                   "PUT /api/accounts/{account_id}/managed;**managed=false 不改变任何既有"
                   "行为**,互动补量/矩阵互动/巡检/采集照旧覆盖全部账号。③代管号笔记数超 "
                   "note_cap 时,每天数据拉取之后自动把「浏览/点赞/收藏/评论/增粉」加权平均"
                   "最低的几篇删掉,全量得分明细落 GET /api/retention-runs 审计流水。"
                   "三条安全轨:发布不足 7 天的不碰、join 不到指标的不碰、单号单日最多删 5 篇;"
                   ...
                   "广播发布**不接受显式 quoted_note_id**(笔记 id 归属单账号,422),"
                   "跨号引用用 related_counselor 逐号推导。",
        "endpoints": [
            "/api/publish-jobs",
            "/api/managed-accounts",
            "/api/accounts/{account_id}/managed",
...
KNOWN_LIMITATIONS = [
    ...
    },
    {
        ...
        "what": "**笔记数量上限判的是我们的台账计数,不是平台真实笔记数**,两者会双向漂移:"
                "①运营在小红书 App / 网页端**手工删过**笔记 —— 台账不会自动少一行"
                "(台账同步补的是新增与字段,没有「平台上没了就删台账行」的逻辑),库存被**虚高**,"
                "可能触发本不该触发的淘汰;②手工发布的笔记要等台账全量同步才会以 orphan 行落库,"
                "同步没跑的窗口期内库存被**低估**,该淘汰的没淘汰。"
                "所以:GET /api/managed-accounts 的 note_count 与你在创作中心看到的篇数对不上"
                "**属已知现象**,不是 bug;差得多时先跑一次 "
                "POST /api/accounts/{account_id}/note-ledger-syncs 再看。"
                "另一条继承自删除链的边界:**删除是按标题定位卡片的**(平台导出无 note_id)。"
                "为此淘汰侧的处置是**同名整批不删**:同一账号有多篇同名笔记时,那几篇一律"
                "不进淘汰候选(审计明细里记「同名歧义跳过」)—— 代价是这几篇永远不参与淘汰,"
                "换掉的是删错人的可能;要让它们能被淘汰,只能先把标题改成不重名的。"
                "还有一条与收敛有关:删除任务落 error(含 server 重启导致的 unknown)时那篇会"
                "重新进候选下轮再试;如果它其实已经删掉了,下一轮会因找不到同题卡片再落一次 "
                "error —— 表现为审计里反复出现同一篇,跑一次台账同步让它落回真实库存即可。",
        "why": "本版**刻意不做平台侧对账**:那要给每个号在淘汰之外再起一次浏览器会话拉全量列表,"
               "而同号一小时会话总闸只有 4 次(实测 5 次就把号弹上验证墙),对账会把预算吃光、"
               "把发布与采集一起饿死。代之以三条护栏把漂移后果压到可接受:单号单日删除封顶 5 篇"
               "(漂移只会让淘汰慢几天,不会一次删错一批)、join 不到指标的笔记一律不删"
               "(平台上已不存在的笔记拿不到新指标)、全量得分明细落审计(事后能查当时按什么数删的)。",
        "since": "2026-08-11",
    },
    {
