...
KNOWN_LIMITATIONS = [
    {
        ...
        "what": "**补挂话题在追加场景仍可能间歇失败,真因只定到一半**:同一个词在第 2 位补得上、"
                "在第 5 位补不上(位置败不是词败),已上线的条件轮询治的是「浮层异步渲染晚到」"
                "这一个候选;另一个候选「话题 chip 累积后光标几何变化、浮层被锚定判据拒错」"
                "**尚未证实也未排除**。所以:补挂结果请逐条看 topics_failed,别当 100% 会成;"
                "reason 是 ``topic_dropdown_not_shown`` / ``topic_dropdown_not_found`` 时"
                "**不要换词**(那是我们的定位/时序问题),把失败条目里的 ``poll_timeline`` "
                "一并反馈过来——它就是为钉死另一个候选而加的取证,**候选一排除就该撤掉**,"
                "别让它变成永久字段。"
                ...
                "同一条保质期纪律。",
        "why": "上一轮把真因定为「``#`` 粘连前一个话题实体」并靠补空格分隔修复,真号复验推翻:"
               "补了空格 tail 确实变成 ``[话题]# #失眠``,浮层照样不弹。轮询是对症的修复"
               "(定长单次快照对晚到的浮层零容错)但尚无真号数据证明它把失败清零。聚焦那半的"
