...
MANIFEST_ENTRIES = [
    {
        ...
        "notes": "**不传 account_id = 广播给全部代管账号**。⚠️ 运营默认口径(老板 08-11 终裁):"
                 "**默认单号定向发布,广播仅用于全矩阵品牌位内容**——同稿六号会与账号定位方案"
                 "串台、且一次吃掉 6 倍每日上限互相蚕食;常规内容请传 account_id 特指。"
                 "(08-10 上线时的「默认齐发」口径已被终裁推翻,勿按旧口径行事。)"
                 "广播不是一条任务发多个号,"
...
class PublishNoteRequest(BaseModel):
    ...
    # 广播语义:不传 account_id 即发全部代管账号。传了就是「特指账号」,行为与上线前一字不变。
    # 运营默认口径注意(08-11 老板终裁):默认单号定向,广播仅限全矩阵品牌位——见 manifest notes。
