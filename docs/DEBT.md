...
| 15 | **商业笔记标记覆盖面抽样**：号7 一篇推介被平台标商业硬禁引用（2026-08-16 两次探针实锤）；全矩阵 118 篇推介是否批量被标未验——若成立"引用推介"玩法整体受限（注：号1/号8 同款推介成功引用过，类别规则已被证伪，是按篇判定） | open | 抽样探针点不同咨询师推介各几张即可判 |
| 16 | 号7 商业笔记 8 次失败记录清出重试队列（error 冷却会反复重试一单永不可能成功的任务） | open | toast 修复合并后处理 |
