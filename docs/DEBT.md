...
| 13 | **自动补量路径 AUTO_WINDOW 散开失效**：2026-08-14 16:12:04 同秒登记 7 张补量单并行执行（done 16:21-16:52 交叠）——"一拥而上"正是调度器 docstring 警告的补量特征形态 | open | 长会话引擎重做编排层时可能直接吸收；先查触发源与 not_before 是否被 worker 尊重。**绊线：2026-08-21 前引擎未上线则单修**——此模式每天在给平台喂同步性指纹，不许无声活到引擎上线 |
