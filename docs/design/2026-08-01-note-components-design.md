...
**补测已查清(2026-08-01,同一篇私密笔记,正文已非空)**:

**① 是追加,不是覆盖。** 正文基线 `#身边的心理学[话题]#`(计数器 7/1000),关联第二个活动后
变成 `#身边的心理学[话题]# #明日方舟[话题]#`(13/1000),**原有正文完整保留**,
新话题以空格分隔拼在末尾。给有正文的笔记关联活动**不会冲掉正文**。

**② 注入的话题名 ≠ 活动名。** 活动叫「明日方舟创作应援」,注入正文的话题是 `#明日方舟`。
话题名是**活动侧配置的,不能从活动名推导**。想预判会注入什么话题,只能点完再读正文。
(上一轮「身边的心理学」恰好活动名与话题名相同,掩盖了这个差异。)

**③ 活动关联是互斥单选,但话题不回收(最危险的一条)。** 点新活动的「关联」后,
旧活动卡自动从「取消关联」变回「关联」,提交载荷里 `ACTIVITY_COMPONENT` 只剩一个 bizId。
**但旧活动注入正文的话题留在原地不删。**

于是:**反复切换活动,话题会单调累积、只增不减,而且这些话题会真的发出去。**
如果实现里有"换个活动重试"的逻辑,跑几次正文就会挂满一串无关话题。
**实现约束:切换活动前必须先确认当前正文里的残留话题,或干脆不做活动重试。**

> 口径:上述取消是"被新活动替换"导致的,**不是**主动点「取消关联」。
> 主动取消会不会清话题,仍未观察。

**④ 首次点击静默失效,第二次才成功。** 第一次点「关联」后 3 秒:正文纹丝不动、
按钮仍是「关联」、无 toast、**零网络请求**,完全没反应。只读诊断已排除禁用态
(`disabled=false`、`pointerEvents=auto`、`opacity=1`、`elementFromPoint` 落在
`.activity-action` 上)。用**完全相同的选择器与点击方式**再点一次立刻成功并弹 toast。

根因未定。可疑点:该按钮很窄(未关联态 `w=28 h=68` 竖排两字),`random_offset`
容错很小;也可能是页面加载完 2.5s 时事件还没绑好。

**实现约束:点完必须校验按钮文案是否翻转成「取消关联」,没翻转就重试。**
否则会静默失败——无异常、无 toast、无网络请求,只会得到一篇没关联活动的笔记还以为成功了。
**这与合集被静默丢弃是同一类问题:这条产品线的失败普遍是静默的。**
