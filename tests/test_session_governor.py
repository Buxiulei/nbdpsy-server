...
- **拦不拦按触发方分**:系统自发任务(operator_id 非正)超帽不派,留队列等下轮重估;
  运营触发任务(operator_id>0)不拦——人工意图优先,运营侧另有配额闸;
...
async def test_operator_job_dispatched_over_cap(db_factory, monkeypatch):
    """运营触发任务(operator_id>0)不拦:同样超帽,人工意图优先照派。"""
    spawned = _install_spawn_recorder(monkeypatch)
    await _seed_browser_sessions(db_factory, 1, 4)
