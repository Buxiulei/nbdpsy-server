...
async def _drive_lifespan_with_interval(tmp_path, monkeypatch, interval):
    ...
    monkeypatch.setattr(worker_mod.settings, "BROWSER_REAP_INTERVAL", interval)

    _Fake = _FakeReaper
    _Fake.instances = []
    monkeypatch.setattr(worker_mod, "BrowserReaper", _Fake)
...
async def test_lifespan_no_reaper_when_interval_zero(tmp_path, monkeypatch):
    """BROWSER_REAP_INTERVAL=0:supervisor 完全不构造 reaper。"""
...
async def test_lifespan_starts_and_stops_reaper_when_positive(tmp_path, monkeypatch):
    """BROWSER_REAP_INTERVAL>0:supervisor 构造并 start,停机时 stop。"""
