...
**引用点击几何根治 + 选篇顺序按老板指令重排。**

- **候选卡不在滚动容器可视区就不点**：真号探针 + 老板人工反证定案——此前"确认钮不解禁/
  平台弹拒绝提示"两类失败的真因是**点击落点在弹窗容器之外**（卡片可视区外 119px），
  不是平台不让引用。新判据取滚动容器矩形上下全判、卡片须完整落区内；滚不进判失败报
  `quote_card_offscreen` 带卡片与容器两组矩形证据，绝不"尽力而为"照点。0.24.4 CHANGELOG
  里「平台裁决/重试无用/该换目标」的描述系误判，已更正——**看到
  `quote_target_not_quotable` / `quote_card_select_not_applied` 不能断定笔记不可引用**。
- **选篇顺序（老板指令）**：零互动笔记永远浮顶（新笔记正处推荐窗口期，早到的赞藏对分发
  有加成），各级内部按真实发布时间新到老。原主键 first_seen_at 是"入台账时刻"，手工
  老笔记会伪装成新帖排前面，与业务意图相反，已换 platform_published_at。**5-6 月老积压
  会沉到队尾最后清，这是设计意图不是卡死。**


## 0.24.4 (2026-08-14)

**引用弹窗第二版修复（真号力学取证驱动）+ 账号级互动日上限。**

- **滚动锚点改用滚动容器本身**：真号探针实测，弹窗里真正可滚的是
  `.select-note-modal__list-wrap`（滚轮从候选卡冒泡上去被它消费），每轮滚动都触发懒加载
  12→22→40、目标排位 35 可达。0.24.3 那版取"页脚上沿之上的候选卡"作锚点，第一轮滚动后
  几何条件失效 → 选不出落点 → 被当成"到底了"，**这才是停在 22 篇的根因**。`exhausted`
  语义同步收严：锚点选不出 ≠ 列表到底。
- **候选卡不在可视区就不点**（新错误码 `quote_card_offscreen`）：真号探针实测目标卡
  `y=727 h=182`、滚动容器可视区 `[184, 608]`——卡在可视区**下方 119px**，点击落点落在弹窗
  之外，根本没点到卡。判据因此从"弹窗页脚上沿"改成 `.select-note-modal__list-wrap` 的矩形，
  **上下都判、要求卡片完整落在里面**；滚满 3 轮仍在区外**不再"尽力而为"照点**，直接判失败
  并把卡片矩形与可视区矩形两组数写进回执。**这才是号 7 那三单确认钮不解禁的真因**：没选中
  任何卡。
- **平台提示不再当成平台裁决**：点候选卡后读 `.d-new-toast`，出现「无法引用」仍报
  `quote_target_not_quotable` 并带原文，但措辞改为中性事实——2026-08-14 人工在同一目标
  （002c0c）上手动引用**成功**，证伪了上一版"这是平台裁决、重试无用、该换目标"的说法。
  该提示可能来自点击落在候选卡之外命中的其它元素，且 toast 容器会累加历史通知，
  **不能据此断定该笔记不可被引用**。只有"点了、没提示、确认钮也没解禁"才报
  `quote_card_select_not_applied`（回执里补了"多为点击没命中候选卡"的指引）。
- **候选覆盖面进失败证据**（`candidates_count` / `candidates_oldest` / `scroll_rounds`）：
  平台给候选列表设了上限（号7 翻到 49 篇 ≈ 上限 50、只覆盖到 2026-02-13，更老的笔记
  **永远翻不到**），与懒加载翻页是**两堵独立的墙**。没有这三个数，"目标不存在"和"目标在
  候选窗口外"分不开——报障方为此绕了一整天。降级到「他人笔记」失败时，这些结构化取证
  也一并合并过去，不再只留一句 reason 文本。
- **账号级互动日上限** `xhs_accounts.interaction_daily_limit`（可空，空或 ≤0 回退全局）：
  用于**恢复期与新号爬坡**——刚被软风控隔离过的号回池就顶满配额是典型行为突变，绝对频次
  没超红线但模式突变本身即风控特征。`PUT /api/accounts/{id}/managed` 可调，**传 0 是清除
  覆盖回退全局，不是"每天 0 篇"**（要完全停互动请置 `restricted`）。


## 0.24.3 (2026-08-13)

**引用弹窗双缺陷修复 + 删除同名硬闸 + 调度死笔记过滤。**

- **引用候选懒加载主动翻页**:候选列表懒加载但原实现从不滚动触发,49 篇的号只收到
  首页 12 张,深位自有笔记被判"不在候选",再被降级门("不在候选=别人的笔记")送进
  必死的他人 tab。现在弹窗内拟人滚动翻页(目标出现/连续无新页/封顶三停),`exhausted`
  语义收严——只有**真翻到底**才允许降级,且降级前先查台账归属,自己的笔记绝不走他人 tab。
- **选卡/确认全链校验**:实证三连败系点击了 disabled 的「确认引用」(夹具原文早存禁用态)。
  现在点卡后回读确认钮解禁(未生效关随机偏移重点一次),点确认前查禁用态,目标卡先滚进
  弹窗视口;新错误码 `quote_card_select_not_applied` / `quote_confirm_disabled` 可判因。
- **删除同名硬闸 `ambiguous_title`**:同题卡 ≥2 张时拒绝不可逆删除(实证:删除按管理页
  DOM 序取首张=最新篇,同题一死一活场景会精确删掉要保留的那篇)。确系"删 N 留 1"清理
  显式带 `allow_ambiguous=true` 放行,manifest 同笔更新。
- **调度过滤 deleted_at**:补量选篇不再给已删笔记派单,与完成率总账口径对齐
  (coverage 交付时点出的分叉,趁早补上)。


## 0.24.2 (2026-08-13)

**互动补量两个断路器:不再白开注定失败的浏览器会话(事故驱动)。** 原有四层闸管的是
"补得多快",管不了"白开多少次注定失败的会话"——而后者烧的是同一份风控预算。

- **事故一(actor 侧)**:号12 自 08-08 起作为互动方 **96 连败**,全部同一个
  `profile_not_loaded`(登录态在、但打开任何发布者主页都渲染不出笔记卡片,账号会话半死)。
  它**没撞验证墙**,所以"撞墙即停 → 置 restricted"那套一次都没触发,风控台账零记录;
  error 冷却一过就再试,96 次白开的会话全是实打实的风控暴露。已人工置 `restricted` 止血。
- **事故二(笔记侧)**:三篇 views=0/0/1 的笔记被**全部 9 个** actor 报 `note_not_found`,
  各积了 16-18 个 error 还在被冷却重试。actor 的报告是准确的——那几篇大概率被平台屏蔽,
  主页根本不展示,再重试一万次也是徒劳。

修法(两个断路器都只在**选篇/选 actor 阶段**判,浏览器层一个字不动:注定失败的会话
压根不该被派出去):

- **断路器A(actor 熔断,带半开探测)**:最近 `INTERACTION_ACTOR_BREAKER_N`(默认 6)条
  台账行全是 error 的号本轮不派活。一篇失败落两行(赞 + 藏),故 6 ≈ 连续三篇整篇失败——
  偶发抖动到不了,半死号一天到得了;单动作失败(赞成了藏没成)会留下 done 行,不算连败。
  **必须有半开**:最新那条 error 超过 `INTERACTION_ACTOR_BREAKER_COOLDOWN_H`(默认 12h)
  就放行一轮,否则熔断即死锁——被熔断的号永不被选中,也就永远产不出成功记录来复位。
  半开那一轮**只做一篇**:按整轮上限(5 篇)放的话,每个冷却周期照旧白开 5 次页,
  断路器就只剩一半意义。成功即自然复位(最近 N 条不再全 error),再失败则从新的那条
  error 重新计时。熔断跳过时按错误类别去重告警一次,不刷屏。
- **断路器B(笔记熔断)**:被 ≥`INTERACTION_NOTE_BREAKER_ACTORS`(默认 3)个**有资格**的
  不同 actor 在发布者主页里翻不到(`note_not_found`)的篇**永久**移出候选。⚠️ **计票必须
  过资格闸**:半死 actor 对**每一篇**都报找不到,票恒为真、不承载信息,裸数票会让 K 变相
  打折(多一个半死号就少要一个真信号)。资格 = `cookie_status='valid'` **且未被断路器A
  熔断**,故**两个断路器结算顺序不能调**(A 先跑,B 才知道谁的票算数)。号12 已不是 valid,
  它那 96 张票据此自动失效。
- ⚠️ **它只判"多个号都翻不到"这个事实,不判成因**。翻不到有两种成因、台账区分不了:被平台
  屏蔽/限流,或它在主页里排得太靠后、超出了定位的滚动预算。两种情况下继续每轮重试都同样
  徒劳,所以一律停调度,成因交给人。**建库时刻实测**:按 K=3 会熔断 10 篇,其中 6 篇
  views 在 67~520 之间(不像被屏蔽),更像后一种成因——上线后请拿 `suppressed_notes`
  逐篇核实;误伤面偏大就调大 K。
- **不自动恢复,但有人工恢复路径**:两种成因都不会自己好,所以熔断的篇不会自己回来。
  `plan_round` 返回 / REST 触发回执 / 调度日志三处都带 `suppressed_notes`——这是**要人去
  核实的清单**,不是错误。核实后确认这篇又该补了,删掉 `note_interactions` 里该 note_id 的
  error 行即可重新入池。票的**自愈**不需要额外逻辑:该表是 `UNIQUE(actor, note, action)`
  覆盖式更新,半死 actor 恢复后重试成功会把自己那两行 error 覆盖成 done,票自动翻转。
- 新配置三项:`INTERACTION_ACTOR_BREAKER_N` / `INTERACTION_ACTOR_BREAKER_COOLDOWN_H` /
  `INTERACTION_NOTE_BREAKER_ACTORS`。**无迁移、无新表**——判据直接查 `note_interactions` 现有行。

**周期性保底台账同步(NoteLedgerSyncScheduler)。** 台账同步原只有发布钩子与手工触发,
纯手工运营号(如禁发的内在关爱号)14 天仅被同步 1 次,手工新发笔记不入册、互动补量根本
看不见。现在全部有效 cookie 的号(不分代管/水军)12 小时保底同步一次,最久未同步者优先、
一轮一单,发布钩子天然刷新时钟故常发号不重复;全局在飞闸让路发布钩子/手工单。每号会话
增量约 2 次/天,远低于小时闸。

**互刷完成率总账端点 `GET /api/interaction-coverage`。** 与调度选篇同口径的唯一权威数源:
每号 公开篇数/应补/已补/待补/完成率、不可调度分解(台账无 note_id 待同步 / 非公开不进
分母)、零互动篇数,actors 出勤欠账表(始终全池),另报 unowned_notes(账号行已移出但笔记
仍在的无主篇,不再静默吞分母)。运营日报接它,终结手拼并集分母。
