...
**受众分析自家号排除修复:退出矩阵的号不再泄漏进潜客漏斗(号9 事故)。** 内容运营
验收当场抓到:号9(米之木木)的账号行已被移出系统,其历史 55 条互刷事件失去认领,以
互动第一名 + 改后昵称「淡三花」顶在漏斗头部——运营按潜客分找人,第一名是自己家的号。

根因:排除名单只查 `xhs_accounts` 活名单,扛得住加号、扛不住删号。修法:新增
`audience_self_userids` **追加型登记表**(进过矩阵就永远排除,只进不出),
`self_account_userids` 每次调用把活名单合并进登记表,排除按登记表走;迁移
`b3d9f2c4a1e7` 种子现存账号并补录号9。昵称漂移实证:**排除键只认 user_id**,任何按
昵称的匹配都会随改名失效。回归测试锁死"删号后仍排除"与"新号自动登记"两个方向。
